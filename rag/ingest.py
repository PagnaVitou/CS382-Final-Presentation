...
class Chunk:
    """Represents a text chunk with metadata for retrieval and display."""
    chunk_id: str
    doc_title: str
    doc_filename: str
    text: str
...
def load_documents(folder: str) -> List[dict]:
    """Load every .txt file in `folder` into {"title": ..., "text": ..., "filename": ...} dicts."""
    docs = []
    for filename in sorted(os.listdir(folder)):
        ...
        docs.append({"title": title, "text": text, "filename": filename})
    return docs
